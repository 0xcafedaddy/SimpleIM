...
public class ToolsParseMsg {
	...
	public static MsgHead parseMsg(byte[] data)throws Exception{
		...
		int dest=dins.readInt();//读取目标yk号
		int src=dins.readInt();//读取源yk号
		...
		if(msgType==IMsgConstance.command_login){//登陆请求
			...
			while(listCount>0){
			  ...
			  byte ykCount=dins.readByte();//组内有几个用户
			  while(ykCount>0){
			  ykCount--;
			  int  ykNum=dins.readInt();//读取一个用户的yk号
		...
		//添加好友
			 ...
			 MsgAddFriend mf=new MsgAddFriend();
				copyHead(msgHead,mf);
				int ykNum=dins.readInt();//要加入的好友的号码
